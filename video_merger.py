...
AUDIO_PATH = "D:\\Yee\\Intern\\RSW\\SQL WEB\\material_audio\\1.mp3"
INPUT_PATH = "D:\\Yee\\Intern\\RSW\\SQL WEB\\material_vid" # Folder containing images and videos
OUTPUT_PATH = "D:\\Yee\\Intern\\RSW\\SQL WEB\\MoneyPrinterPlus\\final"
OUTPUT_FILENAME = "jpg_slideshow.mp4"
...
def convert_image_to_video(image_file, output_dir, duration=5, fps=30, target_width=None, target_height=None, audio_file=None):
    ...
    # Command to convert image to video
        cmd = [
        FFMPEG_PATH,
        '-loop', '1',
        '-i', image_file
...
def merge_videos(input_list, output_file, fps=30, target_width=1080, target_height=1920,
    ...
    if enable_transition and len(processed_video_list) > 1:
        ...
        filter_complex = gen_filter(
            ...
            transition_value,
            transition_duration,
            has_audio  # Include audio transitions
        )
        ...
        ffmpeg_concat_cmd.extend(['-map', '[video]'])
        if has_audio:
            ffmpeg_concat_cmd.extend(['-map', '[audio]'])
        
        ffmpeg_concat_cmd.extend(['-y', output_file])
...
# Example usage for JPEG files only
if __name__ == "__main__":
    # Example with just JPEG files
    # Get all JPEG files from a folder
    folder_path = os.path.normcase(INPUT_PATH)
    
    # Use Path to find all JPEG files
    jpeg_files = []
    for ext in ['.jpg', '.jpeg', '.png']:
        jpeg_files.extend(list(Path(folder_path).glob(f'*{ext}')))
    
    # Convert Path objects to strings
    jpeg_files = [str(file) for file in jpeg_files]
    ...
    background_audio = AUDIO_PATH if os.path.exists(AUDIO_PATH) else None
    
    output = os.path.normcase(OUTPUT_PATH + "\\" + OUTPUT_FILENAME)
    
    # Create slideshow from JPEG files with custom audio
    result = merge_videos(
        jpeg_files,
        output,
        fps=30,
        ...
        image_duration=5,
        audio_file=background_audio  # Add your audio file here
    )
    
    print(f"JPEG slideshow with custom audio saved to: {result}")